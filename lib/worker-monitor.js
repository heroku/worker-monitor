 ...
 * @param {Number,null} [options.logPeriod=10000] the period (in milliseconds)
 *   on which to log the worker's memory usage
 * @param {Number,null} [options.monitorPeriod=10000] the period (in
 *   milliseconds) on which to monitor the worker's memory usage
...
function WorkerMonitor(worker, options) {
   ...
   * @type String
   */
  this.source = cluster.isMaster ? 'master' : 'worker.' + worker.id;
}
